        ...
        <Section title={t.title} icon={<ScrollText />}>
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="flex gap-4 mb-6">
                    <button onClick={() => setStatus('built')} className={`flex-1 p-3 rounded-lg border-2 flex items-center justify-center gap-2 ${status === 'built' ? 'bg-blue-600 text-white border-blue-700' : 'bg-white hover:bg-blue-100'}`}><Building2 /> {t.built}</button>
                    <button onClick={() => setStatus('leased')} className={`flex-1 p-3 rounded-lg border-2 flex items-center justify-center gap-2 ${status === 'leased' ? 'bg-blue-600 text-white border-blue-700' : 'bg-white hover:bg-blue-100'}`}><Handshake /> {t.leased}</button>
                </div>
                ...
                {status && (
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <input name="name" value={formData.name} onChange={handleInputChange} placeholder={t.taxpayerName} className="p-2 border rounded-md"/>
                            <input name="tin" value={formData.tin} onChange={handleInputChange} placeholder={t.tin} className="p-2 border rounded-md"/>
                            <input name="authority" value={formData.authority} onChange={handleInputChange} placeholder={t.authorityName} className="p-2 border rounded-md"/>
                            <input name="address" value={formData.address} onChange={handleInputChange} placeholder={t.address} className="p-2 border rounded-md"/>
                        </div>
                        
                        {/* Step 3 */}
                        <h4 className="font-bold text-lg mb-2">{t.step3}</h4>
                        <div className="relative mb-6">
                            <textarea value={letterText} readOnly className="w-full h-72 p-4 bg-white font-mono text-sm border rounded-md resize-none" />
                            <button onClick={handleCopy} className="absolute top-2 right-2 px-3 py-1 bg-gray-600 text-white text-xs rounded-md hover:bg-gray-700">{copied ? t.copied : t.copy}</button>
                        </div>
                        ...
                        <ul className="space-y-2">
                           {(status === 'built' ? t.checklist.built : t.checklist.leased).map((item, i) => (
                               <li key={i} className="flex items-center p-2 bg-white border rounded-md"><CheckCircle className="h-5 w-5 mr-3 text-green-500"/>{item}</li>
                           ))}
                        </ul>
                    </>
                )}
            ...
            <div className="max-w-5xl mx-auto bg-white shadow-xl rounded-lg">
                <header className="bg-blue-700 text-white p-6 rounded-t-lg relative">
                    <div className="flex items-center">
                        <Briefcase className="h-10 w-10 mr-4 flex-shrink-0" />
                    ...
                    <Section title={t.section3_2_Title} icon={<ListChecks />}>
                        ...
                        <ul className="space-y-2">
                            {t.sources.map((source, i) => (
                                <li key={i} className="flex items-start p-2 bg-gray-50 rounded-md">
